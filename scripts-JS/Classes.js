...
class HealthBar {
    ...
    constructor(color, x, y, hlth, MaxHlth, needOffset = false) {
        this.color = color;
        this.coordinates = {
            x: x,
            y: y
    ...
    draw() {
        ...
        if (!this.needOffset) {
            ctx.fillRect(this.coordinates.x, this.coordinates.y,
                Math.max(0, this.size.width * this.health / this.MaxHealth),
                this.size.height);
        }
        else{
            this.offset = this.size.width - Math.max(0, this.size.width * this.health / this.MaxHealth)
